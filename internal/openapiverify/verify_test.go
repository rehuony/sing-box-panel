...
func TestValidateAcceptsRepositoryContract(t *testing.T) {
	t.Parallel()
	if findings := Validate([]byte(validDocument)); len(findings) != 0 {
		t.Fatalf("Validate(): %s", formatFindings(findings))
	}
}
...
func TestValidateRejectsInvalidContracts(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			t.Parallel()
			findings := Validate([]byte(test.source))
			if !containsFinding(findings, test.message) {
				t.Fatalf("Validate() findings = %s, want a finding containing %q", formatFindings(findings), test.message)
			}
		})
	}
}
...
func TestValidateIsDeterministic(t *testing.T) {
	...
	first := formatFindings(Validate(source))
	second := formatFindings(Validate(source))
	if first != second {
		t.Fatalf("Validate() changed between identical calls:\nfirst:  %s\nsecond: %s", first, second)
	}
}

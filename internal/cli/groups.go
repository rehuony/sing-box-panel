...
func newCoreCommand(state *options, open openApplicationFunc) *cobra.Command {
	root := group("core", "Manage sing-box artifacts, capabilities, and runtime")
	root.AddCommand(
		...
		newCoreCapabilityCommand(state, open),
...
func newConfigCommand(state *options, open openApplicationFunc) *cobra.Command {
	...
	root.AddCommand(
		...
		newConfigManualCommand(state, open),
	)
	root.AddCommand(newConfigRenderCommand(state, open))
	root.AddCommand(newConfigRevisionCommand(state, open))
	return root
}

func newNodeCommand(state *options, open openApplicationFunc) *cobra.Command {
	root := newEntityCommand("node", "Manage canonical nodes", state, open)
	root.AddCommand(newNodeCheckCommand(state, open), newNodeImportCommand(state, open))
	return root
}

func newRuleCommand(state *options, open openApplicationFunc) *cobra.Command {
	return newEntityCommand("rule", "Manage canonical rules", state, open)
}

...
var (
	ErrRunnerAlreadyStarted = errors.New("task runner already started")
	ErrRunnerClosed         = errors.New("task runner is closed")
	ErrRunnerNotStarted     = errors.New("task runner is not started")
)

// Handler performs one claimed task. Calls are serial within a lane and may
...
type Handler interface {
	Handle(context.Context, store.Task, Control) (json.RawMessage, error)
}

// HandlerFunc adapts a function to Handler.
type HandlerFunc func(context.Context, store.Task, Control) (json.RawMessage, error)
...
// TaskStore is the durable boundary the composition root supplies to Runner.
...
// Options defines the worker identity and bounded polling/lease policy.
type Options struct {
	WorkerID          string
...
// Runner owns exactly one goroutine for each durable lane. A short-lived
...
// New constructs a stopped runner. Handler registration is immutable after
// construction so lane goroutines never race on the map.
func New(taskStore TaskStore, handlers map[string]Handler, options Options) (*Runner, error) {
	if taskStore == nil {
...
// cancellation. A Runner is intentionally single-use.
...
func (r *Runner) Wake(lane store.TaskLane) error {
	wake, ok := r.wake[lane]
	if !ok {
...
func (r *Runner) runLane(ctx context.Context, lane store.TaskLane) {
	defer r.wg.Done()
	ticker := r.options.Clock.NewTicker(r.options.PollInterval)
	defer ticker.Stop()
...
func (r *Runner) runTask(ctx context.Context, task store.Task) error {
	...
	heartbeatTicker := r.options.Clock.NewTicker(r.options.HeartbeatInterval)
	go func() {

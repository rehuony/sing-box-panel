...
func TestParseEndpointRequiresLoopbackAndSecret(t *testing.T) {
	...
	if err != nil || valid.BaseURL != "http://127.0.0.1:9090" || valid.Secret != "secret" {
		t.Fatalf("ParseEndpoint(valid) = %+v, %v", valid, err)
	}
	for _, raw := range []string{
		...
		if _, err := ParseEndpoint([]byte(raw)); !errors.Is(err, ErrInvalidConfig) {
			t.Fatalf("ParseEndpoint(%s) error = %v", raw, err)
		}
	}
}
...
func TestClientReadsVersionAndConnectionsWithBearerSecret(t *testing.T) {
	...
	defer server.Close()
	endpoint, err := ParseEndpoint([]byte(fmt.Sprintf(

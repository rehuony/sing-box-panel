...
type runtimeIdentityProcInspector struct {
	root string
}

func runtimeIdentityPlatformInspector() runtimeIdentityProcessInspector {
	return runtimeIdentityProcInspector{root: "/proc"}
}

func (inspector runtimeIdentityProcInspector) Verify(
	ctx context.Context,
...
func (inspector runtimeIdentityProcInspector) ProcessStartToken(ctx context.Context, pid int) (string, error) {
	if pid <= 0 {
...
func (inspector runtimeIdentityProcInspector) procRoot() string {
	if inspector.root == "" {
		return "/proc"
	}

...
func TestInbound114NativeClientConversion(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			result, err := NewInboundRegistry().Convert("1.14.0", subscription.InboundRequest{FinalStartupJSON: input, PublicHost: "203.0.113.7"})
			...
			if err := ValidateConfiguration("1.14.0", document); err != nil {
				t.Fatal(err)
			}
		})
	}

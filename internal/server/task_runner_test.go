...
func TestRunnerCancellationOccursAtExplicitSafePoint(t *testing.T) {
	...
	case err := <-checkpointError:
		if !errors.Is(err, errTaskCancellationRequested) {
			t.Fatalf("SafePoint() error = %v, want errTaskCancellationRequested", err)
		}
	case <-ctx.Done():
...
func (c *fakeClock) NewTicker(interval time.Duration) taskTicker {
	c.mu.Lock()
	defer c.mu.Unlock()
	ticker := &fakeTicker{
		clock:    c,

...
var (
	errTaskCancellationRequested = errors.New("task cancellation requested")
	errTaskSuperseded            = errors.New("task superseded by a newer runtime generation")
)

// taskExecutionControl exposes an explicit safe cancellation boundary to a task handler.
...
type taskExecutionControl interface {
...
func cancellationError(state store.TaskLeaseState) error {
	...
	if state.CancelRequested {
		return errTaskCancellationRequested
	}
	return nil
}

...
func normalizedSourceNodes(values []map[string]any, sourceID string) ([]node.Node, error) {
	if len(values) > node.MaximumNodes {
		return nil, ErrInvalidSource
	}
	nodes := make([]node.Node, 0, len(values))
	...
	for _, value := range values {
		...
		if !node.ValidType(typeID) || !node.ValidTag(tag) || server == "" || !portOK {
			return nil, ErrInvalidSource
		}
	...
	if _, err := node.PublicationDocument(nodes); err != nil {

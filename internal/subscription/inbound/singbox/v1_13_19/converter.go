...
var convertibleInboundTypes = map[string]struct{}{
	...
	"trojan": {}, "naive": {}, "hysteria": {}, "shadowtls": {}, "vless": {},
	"tuic": {}, "hysteria2": {}, "anytls": {},
}
...
// Convert applies only the exact 1.13.19 inbound contract selected by the
// registry; callers cannot request a nearby schema through this converter.
func (converter) Convert(request inbound.Request) (inbound.Result, error) {
	finalStartupJSON := request.FinalStartupJSON
	publicHost := request.PublicHost
	if _, err := normalizedPublicHost(publicHost); err != nil {
		return inbound.Result{}, err
	}
	value, err := document.Decode(finalStartupJSON)
	if err != nil {
		return inbound.Result{}, err
	}
	...
	for index, rawInbound := range inbounds {
		...
		for credentialIndex, credential := range credentials {
			convertedNode, convertErr := convertInboundCredential(
				typeID, tag, publicHost, port, inboundValue, credential, credentialIndex, len(credentials),
			)
			if convertErr != nil {
				diagnostics = append(diagnostics, inboundDiagnostic(index, node.DiagnosticInvalidRequiredField))
				continue

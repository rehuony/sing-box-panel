...
func convert(t *testing.T, document []byte, publicHost string) (inbound.Result, error) {
	t.Helper()
	return New().Convert(inbound.Request{FinalStartupJSON: document, PublicHost: publicHost})
}
...
func TestInbound11319ContractRegistry(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.typeID, func(t *testing.T) {
			...
			if _, err := render.RenderNodes(result.Nodes, render.Channel{Format: render.FormatSingBox}); err != nil {
...
func TestInbound11319ServerOnlyDiagnosticsAndVersionFailClosed(t *testing.T) {
	...
	for index, diagnostic := range result.Diagnostics {
		if diagnostic.Collection != node.CollectionInbounds || diagnostic.ItemIndex != index ||
			diagnostic.Code != node.DiagnosticUnpublishableInbound {
...
func TestInbound11319RejectsTypesAbsentFromTheExactUpstreamRegistry(t *testing.T) {
	...
	if err != nil || len(result.Nodes) != 0 || len(result.Diagnostics) != 1 ||
		result.Diagnostics[0].Code != node.DiagnosticUnsupportedType {
		t.Fatalf("nodes=%+v diagnostics=%+v err=%v", result.Nodes, result.Diagnostics, err)
	}
}
...
func TestInbound11319MultiUserCredentialsHaveSeparateStableGrantKeys(t *testing.T) {
	...
	selected, err := render.RenderNodes(first.Nodes[:1], render.Channel{Format: render.FormatSingBox})
	...
	if !errors.Is(err, inbound.ErrAmbiguousInboundCredential) || len(result.Diagnostics) != 1 || result.Diagnostics[0].Code != node.DiagnosticAmbiguousCredential {

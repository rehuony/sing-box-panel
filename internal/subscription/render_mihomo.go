...
func renderMihomo(values []outbound, diagnostics []Diagnostic) (Result, []Diagnostic) {
	nodes := make([]mihomoNode, 0, len(values))
	for _, value := range values {
		node, code := convertMihomo(value)
		if code != "" {
			diagnostics = append(diagnostics, diagnostic(FormatMihomo, value.collection, value.index, code))
			continue
		}

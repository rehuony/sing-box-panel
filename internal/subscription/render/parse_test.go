...
func TestRenderRejectsStrictJSONViolationsWithSafeCodes(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			t.Parallel()
			_, err := Render(test.data, Channel{Format: FormatSingBox})
...
func TestRenderRejectsSizeAndDepthBoundaries(t *testing.T) {
	...
	_, err := Render(tooLarge, Channel{Format: FormatSingBox})
	assertValidationCode(t, err, ErrInvalidStartup, "document_too_large")

	deep := `{"value":` + strings.Repeat("[", document.MaximumDepth+2) + `0` + strings.Repeat("]", document.MaximumDepth+2) + `}`
	_, err = Render([]byte(deep), Channel{Format: FormatSingBox})
...
func TestRenderRejectsInvalidChannelWithoutParsingInput(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "format", channel: Channel{Format: "clash"}, code: "unsupported_format"},
		{name: "empty tag", channel: Channel{Format: FormatLoon, ExcludeTags: []string{""}}, code: "invalid_tag_exclusion"},
		{name: "duplicate tag", channel: Channel{Format: FormatLoon, ExcludeTags: []string{"a", "a"}}, code: "duplicate_tag_exclusion"},
		{name: "invalid type", channel: Channel{Format: FormatLoon, ExcludeTypes: []string{"VMess"}}, code: "invalid_type_exclusion"},
		{name: "duplicate type", channel: Channel{Format: FormatLoon, ExcludeTypes: []string{"vmess", "vmess"}}, code: "duplicate_type_exclusion"},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			t.Parallel()

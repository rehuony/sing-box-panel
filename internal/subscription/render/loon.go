...
func renderLoon(values []outbound, diagnostics []Diagnostic) (Result, []Diagnostic) {
	nodes := make([]loonNode, 0, len(values))
	for _, value := range values {
		node, code := convertLoon(value)
		if code != "" {
			diagnostics = append(diagnostics, diagnostic(FormatLoon, value.collection, value.index, code))
			continue
		}

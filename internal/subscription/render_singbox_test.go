...
func TestRenderSingBoxSortsFiltersAndClosesDependencies(t *testing.T) {
	...
	wantDiagnostics := []RenderDiagnostic{
		diagnostic(RenderFormatSingBox, CollectionOutbounds, 4, DiagnosticUnresolvedDependency),
		diagnostic(RenderFormatSingBox, CollectionOutbounds, 5, DiagnosticDuplicateTag),
		diagnostic(RenderFormatSingBox, CollectionOutbounds, 6, DiagnosticDuplicateTag),
		diagnostic(RenderFormatSingBox, CollectionOutbounds, 7, DiagnosticInvalidOutbound),
	}
	if !reflect.DeepEqual(result.Diagnostics, wantDiagnostics) {
		t.Fatalf("Diagnostics = %#v, want %#v", result.Diagnostics, wantDiagnostics)
	}
	filtered, err := Render(startup, RenderChannel{Format: RenderFormatSingBox, ExcludeTags: []string{"alpha"}})
	if err != nil {

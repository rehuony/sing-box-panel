...
func TestFetchRejectsPrivateByDefaultAndAllowsExplicitCIDR(t *testing.T) {
	...
	if _, err := FetchSource(context.Background(), server.URL, nil); !errors.Is(err, ErrSourceAddressDenied) {
		t.Fatalf("default private address error=%v", err)
	}
	body, err := FetchSource(context.Background(), server.URL, []string{"127.0.0.1/32"})
...
func TestFetchRevalidatesRedirectTargetAndHidesCredentials(t *testing.T) {
	...
	_, err := FetchSource(context.Background(), withCredentials, []string{"127.0.0.1/32"})
	if !errors.Is(err, ErrSourceAddressDenied) || strings.Contains(err.Error(), "super-secret") {
...
func TestFetchBoundsRedirectsAndBody(t *testing.T) {
	...
	if _, err := FetchSource(context.Background(), redirects.URL+"/0", []string{"127.0.0.1/32"}); !errors.Is(err, ErrTooManySourceRedirects) {
		t.Fatalf("redirect error=%v", err)
	}

	large := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		_, _ = w.Write(bytes.Repeat([]byte{'x'}, MaximumSourceBodyBytes+1))
	}))
	t.Cleanup(large.Close)
	if _, err := FetchSource(context.Background(), large.URL, []string{"127.0.0.1/32"}); !errors.Is(err, ErrSourceBodyTooLarge) {

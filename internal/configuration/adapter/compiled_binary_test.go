...
func TestCompiledAdaptersAcceptExactOfficialBinary(t *testing.T) {
	...
	projection, err := registry.Project(profile, adapter.Request{
		CanonicalJSON: []byte(`{"schema_version":2,"configuration":{}}`),
	})
	if err != nil {
		t.Fatalf("project empty canonical configuration: %v", err)
	}

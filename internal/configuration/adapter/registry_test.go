...
func (current testAdapter) Provenance() Provenance {
	return Provenance{UpstreamTag: "v" + current.version, UpstreamCommit: "commit", Source: "test"}
}
func (current testAdapter) Supports(profile Profile) bool {
	return profile.ExactVersion == current.version
}
func (current testAdapter) Project(Request) (Result, error) {
	return FinalizeResult([]byte(`{}`), nil)
}

func TestRegistryUsesOnlyExactValidatedProfiles(t *testing.T) {
	t.Parallel()

	registry := MustNewRegistry(testAdapter{version: "1.13.19"}, testAdapter{version: "1.11.15"})
...
func TestIgnoredDigestRequiresExactAcceptance(t *testing.T) {
	...
	result, err := FinalizeResult([]byte(`{"log":{}}`), []Diagnostic{{

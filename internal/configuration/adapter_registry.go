...
// MustNewAdapterRegistry constructs an immutable compile-time registry. Invalid or
// duplicate registrations are programmer errors and fail during composition.
func MustNewAdapterRegistry(adapters ...Adapter) *AdapterRegistry {
	...
	return &AdapterRegistry{byVersion: byVersion}
}

func (registry *AdapterRegistry) Resolve(profile CoreProfile) (Adapter, error) {
...
func (registry *AdapterRegistry) Project(profile CoreProfile, request ProjectionRequest) (ProjectionResult, error) {
...
func (registry *AdapterRegistry) Versions() []string {
	if registry == nil {
		return []string{}
	}

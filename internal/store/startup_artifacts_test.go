...
func TestStartupArtifactBindsRawConfigurationToRevisionAndExactBinary(t *testing.T) {
	...
	core, err := database.UpsertCoreArtifact(ctx, CoreArtifact{
		ID: "core-raw", ExactVersion: "1.13.19", OperatingSystem: "linux", Architecture: "amd64", Variant: "musl",
...
func TestCompiledRawStartupRechecksHeadBeforeInsert(t *testing.T) {
	...
	first, err := saveTestConfiguration(ctx, database, 0, NewCanonicalRevision{
	...
	core, err := database.UpsertCoreArtifact(ctx, CoreArtifact{
		ID: "core-stale", ExactVersion: "1.13.19", OperatingSystem: "linux", Architecture: "amd64", Variant: "musl",
	...
	_, err = saveTestConfiguration(ctx, database, 1, NewCanonicalRevision{
	...
	_, err = database.CreateCompiledStartupArtifact(ctx, StartupArtifact{

...
func (document *Document) ValueAtPointer(pointer string) (any, error) {
	if document == nil {
		return nil, errors.New("canonical document is nil")
	}
	tokens, err := parseEditPointer(pointer)
	if err != nil {
		return nil, err
	}
	var current any = document.Map()
...
func (document *Document) SetPointer(pointer string, value any) (*Document, error) {
	...
	if len(tokens) == 0 {
		...
		if !ok {
			return nil, errors.New("canonical root replacement must be an object")
		}
	...
	case []any:
		...
		typed[index] = cloned
	default:
		return nil, fmt.Errorf("%w: parent is not a container", ErrPointerNotFound)
	}
...
func (document *Document) UnsetPointer(pointer string) (*Document, error) {
	...
	if len(tokens) == 0 {
		return nil, errors.New("canonical root cannot be removed")
	}
	root := document.Map()
	parent, err := pointerParent(root, tokens[:len(tokens)-1])
	if err != nil {
	...
	case []any:
		...
		updated := append(typed[:index], typed[index+1:]...)
		if err := replaceArrayAtParent(root, tokens[:len(tokens)-1], updated); err != nil {
			return nil, err
		}
	default:
		return nil, fmt.Errorf("%w: parent is not a container", ErrPointerNotFound)

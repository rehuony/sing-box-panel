    ...
    <div className="page-stack">
      ...
      <section className="instrument-strip" aria-labelledby="metrics-title">
        <div className="instrument-strip__identity">
          ...
          <span className={`state-label ${metrics?.available ? 'state-label--success' : 'state-label--warning'}`}>
            <span aria-hidden="true" />{metrics?.available ? 'Available' : 'Unavailable'}
          </span>
        </div>
        {metrics === null ? <div className="inline-loading" aria-busy="true">Reading collector evidence…</div> : metrics.available && current ? (
          <>
            <div className="instrument-reading"><span>Inbound</span><strong>{formatBytes(current.inbound_bytes)}</strong><small>Measured in current period</small></div>
            <div className="instrument-reading"><span>Outbound</span><strong>{formatBytes(current.outbound_bytes)}</strong><small>Measured in current period</small></div>
            <div className="instrument-reading"><span>Bundle</span><strong>{metrics.applied_bundle_id ?? 'Unidentified'}</strong><small>{metrics.monitoring_tier ?? 'Unknown'} monitoring</small></div>
          </>
        ) : (
      ...
      <div className="observability-layout">
        ...
        <section className="traffic-panel" aria-labelledby="traffic-periods-title">
          <div className="section-heading"><div><p className="eyebrow">Recorded intervals</p><h2 id="traffic-periods-title">Traffic periods</h2></div><span>{traffic?.monitoring_tier ?? 'unavailable'} tier</span></div>
          {periodError === '' ? null : <div className="form-error" role="alert"><strong>Period detail failed</strong><span>{periodError}</span></div>}
          {periods === null ? <div className="inline-loading" aria-busy="true">Loading persisted periods…</div> : null}
          {periods?.length === 0 ? <div className="empty-state"><strong>No persisted periods.</strong><p>This is not zero traffic. No collector sample is available for the selected history.</p></div> : null}
          {periods && periods.length > 0 ? (
            <ol className="traffic-period-list">
              {periods.map((period) => <li key={period.id}><button aria-pressed={selectedPeriod?.id === period.id} onClick={() => void inspectPeriod(period)} type="button"><span><strong>{formatTime(period.period_start)}</strong><small>until {formatTime(period.period_end)}</small></span><span><b>↓ {formatBytes(period.inbound_bytes)}</b><b>↑ {formatBytes(period.outbound_bytes)}</b></span></button></li>)}
            </ol>
          ) : null}
          {selectedPeriod === null ? null : (
            <div className="period-detail" aria-live="polite">
              <div className="section-heading"><div><p className="eyebrow">Exact persisted record</p><h3>{selectedPeriod.id}</h3></div><span>{selectedPeriod.activation_bundle_id ?? 'No bundle ID'}</span></div>
              <dl><div><dt>Inbound bytes</dt><dd>{selectedPeriod.inbound_bytes.toLocaleString()}</dd></div><div><dt>Outbound bytes</dt><dd>{selectedPeriod.outbound_bytes.toLocaleString()}</dd></div><div><dt>Created</dt><dd>{formatTime(selectedPeriod.created_at)}</dd></div></dl>
              <details><summary>Counter breakdown</summary><pre>{JSON.stringify(selectedPeriod.counters, null, 2)}</pre></details>
            </div>
          )}
        </section>
      </div>

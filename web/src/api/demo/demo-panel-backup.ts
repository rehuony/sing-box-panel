...
export function demoBackupSettings(view: PanelSettingsView, secrets: DemoPanelSecrets) {
  ...
  return {
    ...
    subscription: {
      author: s.subscription_author,
      provider: s.subscription_provider,
      private_source_cidrs: s.private_source_cidrs,
    },
    logs: {
      retention_days: s.log_retention_days ?? 0,
  ...
  return {
    ...
    view: {
      ...
      service: {
        ...
        subscription_author: native.subscription.author,
        subscription_provider: native.subscription.provider,
        private_source_cidrs: native.subscription.private_source_cidrs,
        log_retention_days: native.logs.retention_days ?? 0,
